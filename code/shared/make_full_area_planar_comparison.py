...
from io_paths import FULL_AREA_DIR, FULL_AREA_GEOJSON_DIR, FULL_AREA_IMAGE_DIR, PPT_DIR, PPT_IMAGE_DIR, PPT_ZIP, PROJECT_DIR, RESULTS_DIR
...
OUT_PNG = FULL_AREA_IMAGE_DIR / f"{DATE}_fig_full_area_planar_method_vs_gamma.png"
...
def summarize_text(rows: list[dict], distances: np.ndarray, fids: np.ndarray) -> str:
    ...
    if stats_rows:
        ...
        if method_keys:
            ...
            mean_method = f"\n建筑约束方法边界均值：{np.mean(vals):.2f} 米"
        if gamma_keys:
            vals = [float(r[gamma_keys[0]]) for r in stats_rows if r.get(gamma_keys[0], "")]
            mean_gamma = f"\nGAMMA/DSM 边界均值：{np.mean(vals):.2f} 米"
    return (
        f"日期：{DATE}\n"
        f"建筑数：{valid_buildings}\n"
        f"点对数：{len(rows):,}\n"
        f"点对距离中位数：{np.median(distances):.2f} 米\n"
        f"点对距离 P90：{np.percentile(distances, 90):.2f} 米"
...
def make_figure() -> None:
    ...
    axes[0].set_title("建筑约束方法", fontsize=12, weight="bold")
    ...
    axes[1].set_title("传统 GAMMA/DSM 方法", fontsize=12, weight="bold")
    ...
    axes[2].scatter(gamma[sampled_points, 0], gamma[sampled_points, 1], s=1.0, c="#f97316", alpha=0.34, linewidths=0, label="传统 GAMMA/DSM")
    axes[2].scatter(method[sampled_points, 0], method[sampled_points, 1], s=1.0, c="#2563eb", alpha=0.52, linewidths=0, label="建筑约束方法")
    ...
    axes[2].set_title("平面叠加与抽样点对连线", fontsize=12, weight="bold")

    for ax in axes:
        set_equal_lonlat(ax, extent)

    fig.suptitle("全区域 SAR 建筑地理编码方法平面对比", fontsize=15, weight="bold")
...
def update_zip() -> None:
    ...
    with zipfile.ZipFile(tmp, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for root in [PPT_DIR, PPT_IMAGE_DIR, RESULTS_DIR / "trash"]:
            if not root.exists():
                continue
