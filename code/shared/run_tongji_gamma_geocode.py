...
KEEP_MAIN_IMAGES = {
    ...
    "fig_09_zoomed_geocoded_points.png",
}


def prune_main_images() -> None:
    for path in MAIN_IMAGE_DIR.glob("*.png"):
        if path.name not in KEEP_MAIN_IMAGES:
            path.unlink()
...
def write_local_runner() -> None:
    ...
    runner.write_text(
        ...
        f"/home/u/miniconda3/envs/sar-geocode/bin/python {shared_script} \"$@\" 2>&1 | tee results/logs/main/run.log\n",
        encoding="utf-8",
    )
    runner.chmod(0o755)
...
def write_local_readme(dirs, processed_dates, all_stats, missing) -> None:
    ...
    lines = [
        "# a_geo_huajiachi: Huajiachi GAMMA 建筑物约束 SAR 精细地理编码",
        "",
        "## 目录约定",
        "- `data/`: 原始输入数据，只读使用。",
        "- `code/`: 本次整理后的实验脚本。",
        ...
        f"- Huajiachi 建筑物矢量: `{BUILDINGS_SHP}`",
        f"- Huajiachi DSM: `{DSM_TIF}`",
        f"- {shp_fields_note()}",
        "",
        "## 方法说明",
        ...
        "## 运行",
        "```bash",
        "cd /home/u/geocoding/geo_hangzhou/geo_bc/a_geo_tongji",
        "bash run.sh",
        "```",
        "",
        "## 输出",
        f"- 图片: `results/images/main/`",
        f"- GeoTIFF: `results/rasters/main/`",
        f"- 散射点与误差 CSV: `results/tables/main/`",
        f"- 建筑物和散射点 GeoJSON: `results/geodata/main/`",
        f"- 日志: `results/logs/main/run.log`",
        "",
        "## 关键论文图",
        "- `results/images/main/fig_01_huajiachi_sar_intensity_with_buildings.png`",
        "- `results/images/main/fig_02_initial_projection_masks.png`",
        "- `results/images/main/fig_03_refined_masks.png`",
        "- `results/images/main/fig_04_initial_vs_refined_masks.png`",
        "- `results/images/main/fig_05_method_geocoded_points.png`",
        "- `results/images/main/fig_06_gamma_vs_proposed_map.png`",
        "- `results/images/main/fig_07_error_statistics.png`",
        "- `results/images/main/fig_08_3d_scatter_points.png`",
        "- `results/images/main/fig_09_zoomed_geocoded_points.png`",
        "",
        "## 本次结果摘要",
...
def make_polished_geocode_figures(date: str = "20250109") -> None:
    figures = MAIN_IMAGE_DIR
    ...
    ax.scatter(gamma[:, 0], gamma[:, 1], s=1.4, c="#f28e2b", alpha=0.32, linewidths=0, label="传统 GAMMA/DEM", zorder=5)
    sc = ax.scatter(method[:, 0], method[:, 1], s=1.8, c=method[:, 2], cmap="viridis", alpha=0.82, linewidths=0, label="建筑约束方法", zorder=6)
    ax.set_xlim(*xlim)
    ax.set_ylim(*ylim)
    ax.set_aspect("equal", adjustable="box")
    ax.set_xlabel("经度 / 度")
    ax.set_ylabel("纬度 / 度")
    ax.set_title("华家池建筑约束 SAR 地理编码")
    ...
    out = figures / "fig_06_gamma_vs_proposed_map.png"
    ...
    ax.scatter(target_gamma[:, 0], target_gamma[:, 1], s=5, c="#f28e2b", alpha=0.45, linewidths=0, label="传统 GAMMA/DEM", zorder=5)
    ax.scatter(target_method[:, 0], target_method[:, 1], s=6, c="#2c7fb8", alpha=0.88, linewidths=0, label="建筑约束方法", zorder=6)
    ax.set_xlim(*zx)
    ax.set_ylim(*zy)
    ax.set_aspect("equal", adjustable="box")
    ax.set_xlabel("经度 / 度")
    ax.set_ylabel("纬度 / 度")
    ax.set_title(f"建筑 {target} 地理编码点放大图")
    ax.ticklabel_format(useOffset=False, style="plain")
    ax.grid(color="white", alpha=0.24, linewidth=0.25)
    ax.legend(loc="upper right", frameon=True, framealpha=0.92)
    fig.tight_layout(pad=0.25)
    zoom = figures / "fig_09_zoomed_geocoded_points.png"
...
def main() -> None:
    ...
    core.main()
    if os.environ.get("SKIP_POLISHED_GEOCODE_FIGURES") != "1":
        make_polished_geocode_figures("20250109")
    prune_main_images()
    ...
    log(f"a_geo_huajiachi summary: {json.dumps(summary, ensure_ascii=False)}")
