...
POINTS_CSV = FULL_AREA_DIR / f"{DATE}_all_buildings_method_vs_gamma_points.csv"
STATS_CSV = FULL_AREA_DIR / f"{DATE}_all_buildings_fig5_4_like_stats.csv"
...
def fig_vector_field(method: np.ndarray, gamma: np.ndarray, buildings: dict[int, dict]) -> Path:
    ...
    cbar.set_label("点对距离中位数 / 米")
    ax.set_title("全区域位移矢量场：GAMMA/DSM 至建筑约束结果", fontsize=13, weight="bold")
    ax.text(0.02, 0.02, "箭头方向：传统 GAMMA/DSM 点 → 建筑约束点",
    ...
    fig.savefig(out)
    plt.close(fig)
    shutil.copy2(out, FULL_AREA_IMAGE_DIR / out.name)
    return out
...
def fig_error_heatmap(method: np.ndarray, gamma: np.ndarray, buildings: dict[int, dict]) -> Path:
    ...
    cbar.set_label("点对距离中位数 / 米")
    ax.set_title("全区域平面差异热力图", fontsize=13, weight="bold")
    ax.text(0.02, 0.02, "六边形网格颜色表示同一 SAR 抽样像素两种结果之间的距离中位数。",
            transform=ax.transAxes, fontsize=8, bbox={"boxstyle": "round,pad=0.28", "fc": "white", "ec": "#9ca3af", "alpha": 0.9})
    fig.tight_layout()
...
def fig_zoom_cases(method: np.ndarray, gamma: np.ndarray, fids: np.ndarray, buildings: dict[int, dict], stats: list[dict]) -> Path:
    ...
    for ax, fid in zip(axes.ravel(), chosen):
        ...
        ax.scatter(gamma_xy[sk, 0], gamma_xy[sk, 1], s=13, color="#f97316", alpha=0.62, linewidths=0, label="传统 GAMMA/DSM")
        ax.scatter(method_xy[sk, 0], method_xy[sk, 1], s=13, color="#2563eb", alpha=0.80, linewidths=0, label="建筑约束方法")
        ax.set_xlim(extent[0], extent[1])
        ax.set_ylim(extent[2], extent[3])
        ax.set_aspect("equal", adjustable="box")
        ax.set_xlabel("东向偏移 / 米", fontsize=8)
        ax.set_ylabel("北向偏移 / 米", fontsize=8)
        ax.grid(color="#9ca3af", linewidth=0.3, alpha=0.28)
        ax.tick_params(labelsize=7)
        stat = next((r for r in stats if int(r["fid"]) == fid), None)
        title = f"建筑编号 {fid}"
        if stat:
            title += f"｜GAMMA 均值 {float(stat['gamma_dsm_mean_boundary_distance_m']):.1f} 米"
        ax.set_title(title, fontsize=9, weight="bold")
    axes.ravel()[0].legend(loc="best", fontsize=7, markerscale=1.5)
    fig.suptitle("GAMMA/DSM 边界误差较大的典型建筑", fontsize=14, weight="bold")
    fig.tight_layout(rect=[0, 0, 1, 0.94])
    out = FIG_DIR / "fig_12_typical_building_zoom_planar_comparison.png"
    fig.savefig(out)
    plt.close(fig)
    shutil.copy2(out, FULL_AREA_IMAGE_DIR / out.name)
    return out
...
def fig_ranked_improvement(stats: list[dict]) -> Path:
    ...
    ax.invert_yaxis()
    ax.set_xscale("log")
    ax.set_xlabel("到建筑轮廓边界的平均距离 / 米（对数尺度，越低越好）")
    ax.set_ylabel("建筑编号")
    ax.set_title("GAMMA/DSM 误差最大的 30 栋建筑边界距离改善", fontsize=13, weight="bold")
    ax.text(
        0.02,
        0.985,
        f"这 30 栋建筑的均值：GAMMA/DSM {np.mean(gamma):.2f} 米 → 建筑约束方法 {np.mean(method):.3f} 米",
    ...
    ax.legend(loc="lower right", fontsize=8, frameon=True, title="点的含义")
...
def fig_height_error_relation(stats: list[dict]) -> Path:
    ...
    axes[0].scatter(
        ...
        edgecolor="white",
        linewidth=0.25,
        label="传统 GAMMA/DSM 边界距离",
    )
    axes[0].axhline(float(np.mean(gamma)), color="#9a3412", linestyle="--", linewidth=1.0, label=f"GAMMA/DSM 均值 = {np.mean(gamma):.2f} 米")
    axes[0].set_title("传统 GAMMA/DSM\n（建筑约束前的边界距离）", fontsize=11, weight="bold", pad=8)
    axes[0].set_ylabel("到建筑轮廓边界的平均距离 / 米")
    ...
    axes[1].axhline(float(np.mean(method)), color="#1d4ed8", linestyle="--", linewidth=1.0, label=f"建筑约束均值 = {np.mean(method):.2f} 米")
    axes[1].set_title("建筑约束方法\n（约束后的边界距离；纵轴放大）", fontsize=11, weight="bold", pad=8)
    axes[1].set_ylabel("到建筑轮廓边界的平均距离 / 米")
    axes[1].set_ylim(-0.05, max(2.2, float(np.percentile(method, 98)) * 1.25))

    for ax in axes:
        ax.set_xlabel("建筑高度 / 米")
    ...
    for level in size_levels:
        ...
        labels.append(f"{level} 个 SAR 样本")
    method_legend = axes[1].legend(loc="upper right", fontsize=8, frameon=True, title="方法线与点")
    axes[1].add_artist(method_legend)
    axes[1].legend(handles=handles, labels=labels, loc="lower right", fontsize=8, frameon=True, title="点大小")

    fig.suptitle("建筑高度与边界距离的关系", fontsize=13.5, weight="bold", y=0.985)
...
def fig_cdf(stats: list[dict]) -> Path:
    ...
    ax.plot(method, y_m, color="#2563eb", linewidth=2.0, label="建筑约束方法")
    ax.set_xscale("symlog", linthresh=0.01)
    ax.set_xlabel("平均边界距离 / 米")
    ax.set_ylabel("建筑累计比例")
    ax.set_title("建筑级边界误差累计分布", fontsize=13, weight="bold")
...
def write_doc(extra_figs: list[Path], stats: list[dict], method: np.ndarray, gamma: np.ndarray) -> Path:
    ...
    lines = [
        ...
        "## 总体口径",
        "",
        "本图件包分为两组：第一组是 PS-InSAR 同像素典型建筑对比，第二组是 Huajiachi 全区域建筑物约束地理编码与传统 GAMMA/DSM 地理编码的平面对比。全区域图使用同一批 SAR 采样像素，分别给出本文建筑物约束方法坐标和传统 GAMMA/DSM 坐标。",
        "",
        "## 全区域关键统计",
        "",
        f"- 有效建筑物：{len(stats)} 栋。",
        ...
        "## 推荐 PPT 使用顺序",
        "",
        "1. `20250109_fig_full_area_planar_method_vs_gamma.png`：全区域三联平面对比，先建立整体视觉结论。",
        ...
        "4. `fig_12_typical_building_zoom_planar_comparison.png`：用局部建筑案例解释点云如何从 GAMMA/DSM 结果回到建筑轮廓附近。",
        ...
        "- `fig_12_typical_building_zoom_planar_comparison.png`：选取传统 GAMMA/DSM 轮廓边界误差较大的 6 栋建筑做局部放大。灰色为建筑轮廓，橙色为 GAMMA/DSM 点，蓝色为建筑物约束点，连线表示同一 SAR 像素的两种反算结果。",
        ...
        "## 已有图件说明",
        "",
        "- `fig_01_sar_overview_selected_buildings_pixels.png`：SAR 幅度底图上的典型建筑投影三角网和同像素 PS 点。",
        "- `fig_02_per_building_sar_crops.png`：典型建筑 SAR 局部放大图。",
        "- `fig_03_sar_pixel_density.png`：典型建筑同像素 PS 点密度图。",
        "- `fig_04_geographic_same_pixel_comparison.png`：典型建筑地理坐标下 PS/GAMMA 与建筑约束点对比。",
        ...
        "- `fig_same_ps_pixel_3d_buildings.png`：典型建筑同像素三维对比。",
        "- `fig_same_ps_pixel_radar_pixels.png`：典型建筑 SAR 行列坐标投影图。",
        "",
        "## 结论表述建议",
        "",
